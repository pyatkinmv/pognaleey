...
public class TravelGuideContentProviderV1 extends TravelGuideContentProvider {
  ...
  public TravelGuideContentProviderV1(
      ImageService imageService,
      TravelInquiryService inquiryService,
  ...
  private static String enrichGuideWithTitleImage(
      String guideContent, @Nullable String title, String titleImageUrl) {
    if (title == null) {
      return guideContent;
    }
  ...
  /**
   * Extracts the title from the guide content by removing the curly braces `{}` around the first
   * substring found. If no such substring is found, logs a warning and returns `null`.
   *
   ...
   *     <p>Example usage: {@code resolveGuideTitle("# {Guide Title} text");} returns {@code "Guide
   *     Title"}. {@code resolveGuideTitle("No braces here");} returns null.
   */
  @Nullable
  ...
  private static String enrichWithContentImages(
      String guideDetailsWithoutImages, Map<String, ImageDto> titleToImageMap) {
  ...
  void enrichGuideWithContent(
    ...
    try {
      ...
      var guideImagesPrompt =
          promptService.generateGuideImagesPrompt(recommendationTitle, inquiry.getParams());
