...
public class ImageService {
    ...
    public List<ImageDto> saveAll(List<ImageDto> dtos) {
        var images = dtos.stream()
                .map(TravelMapper::toImage)
                .toList();
        return imageRepository.saveAllFromIterable(images)
                .stream()
                .map(TravelMapper::toImageDto)
                .toList();
    }
    ...
    public ImageDto findByIdOrThrow(Long imageId) {
        return imageRepository.findById(imageId).map(TravelMapper::toImageDto).orElseThrow(() -> new EntityNotFoundException(imageId, Image.class));
    }
    ...
    public Map<Long, ImageDto> getIdToImageMap(Set<Long> imagesIds) {
        ...
        return StreamSupport.stream(imageRepository.findAllById(imagesIds).spliterator(), false).map(TravelMapper::toImageDto).collect(Collectors.toMap(ImageDto::id, it -> it));
    }
    ...
    //        var width = 1024;
    //        var q = 1.0;
    //        String path = String.format("%s%s", "C:\\Users\\maxim\\IdeaProjects\\pognaleey\\", filename);
    //        String output = String.format("%s%s-w%d-q%f.%s", "C:\\Users\\maxim\\IdeaProjects\\pognaleey\\", filename, width, q, outputExt);
    //
