...
public class OpenverseImageSearchHttpClient extends ImageSearchHttpClient<OpenverseImagesResponseDto> {
    ...
    @Override
    Optional<ImageSearchClientImageDto> retrieveTargetImages(OpenverseImagesResponseDto response, String searchQuery) {
        return response.results()
                .stream()
                .map(it -> new ImageSearchClientImageDto(it.url(), it.thumbnail(), searchQuery, it.license_url(),
                        it.creator(), it.creator_url()))
                .findFirst();
    }
