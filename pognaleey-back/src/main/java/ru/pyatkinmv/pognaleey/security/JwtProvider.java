...
public class JwtProvider {
  ...
  public String generateToken(
      Long userId, String username, Collection<? extends GrantedAuthority> roles) {
    var claims = Jwts.claims().setSubject(username.toLowerCase());
    claims.put(
        "roles", roles.stream().map(GrantedAuthority::getAuthority).collect(Collectors.toList()));
    ...
    return Jwts.builder()
        ...
        .setExpiration(
            new Date(System.currentTimeMillis() + MILLIS_IN_MINUTE * tokenValidityTimeMinutes))
