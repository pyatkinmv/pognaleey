...
class TravelGuideServiceTest extends DatabaseCleaningTest {
  @Autowired private TravelGuideService travelGuideService;
  @Autowired private TravelGuideRepository travelGuideRepository;
  @Autowired private UserService userService;
  @Autowired private AuthService authService;
  @Autowired private UserRepository userRepository;
  @Autowired private TravelInquiryRepository travelInquiryRepository;
  @Autowired private TravelRecommendationRepository travelRecommendationRepository;
  ...
  void createGuide() {
    ...
    assertThrows(
        RuntimeException.class, () -> travelGuideService.createGuide(recommendation.getId()));

    var anotherRecommendation = createRecommendation();
    var userGuide =
        withUser("user-1", () -> travelGuideService.createGuide(anotherRecommendation.getId()));
  ...
  void likeGuide() {
    ...
    assertThrows(RuntimeException.class, () -> travelGuideService.likeGuide(guide.getId()));
  ...
  void unlikeGuide() {
    var guide = createTravelGuide();
    assertThrows(RuntimeException.class, () -> travelGuideService.unlikeGuide(guide.getId()));
  ...
  void getMyGuides() {
    ...
    assertThrows(RuntimeException.class, () -> travelGuideService.getMyGuides(Pageable.ofSize(10)));

    var userGuidesPage =
        withUser("user-1", () -> travelGuideService.getMyGuides(Pageable.ofSize(10)));
  ...
  void getLikedGuides() {
    ...
    assertThrows(
        RuntimeException.class, () -> travelGuideService.getLikedGuides(Pageable.ofSize(10)));
  ...
  private <T> T withUser(String username, Supplier<T> supplier) {
    ...
    var authentication =
        new JwtAuthenticationToken(user.getId(), user.getUsername(), user.getAuthorities());
    ...
    var image =
        imageRepository.save(
            new Image(
                ...
                null));
    var recommendation =
        travelRecommendationRepository.save(
